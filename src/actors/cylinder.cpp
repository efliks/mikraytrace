...
SimpleCylinder::SimpleCylinder(const StandardBasis& local_basis, double radius, 
        double length, shared_ptr<TextureMapper> texture_mapper_ptr) : 
...
void create_cylinder(TextureFactory* texture_factory,
                     shared_ptr<ConfigTable> cylinder_items,
                     std::vector<shared_ptr<ActorBase> >* actor_ptrs) 
{
    ...
    shared_ptr<ActorBase> cylinder_ptr = shared_ptr<ActorBase>(new SimpleCylinder(
        cylinder_basis,
        cylinder_radius,
        cylinder_span,
        texture_mapper_ptr
